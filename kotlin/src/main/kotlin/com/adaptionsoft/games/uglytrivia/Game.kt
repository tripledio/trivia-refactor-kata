...
class Game(internal var presenter: Presenter = SystemPresenter()) {
    ...
    fun add(playerName: String): Boolean {
        ...
        presenter.present("They are player number " + players.size)
        return true
    }
    ...
    fun roll(roll: Int) {
        presenter.present(players[currentPlayer].toString() + " is the current player")
        ...
        if (inPenaltyBox[currentPlayer]) {
            if (roll % 2 != 0) {
                ...
                presenter.present(players[currentPlayer].toString() + " is getting out of the penalty box")
                ...
                presenter.present(players[currentPlayer].toString() + " is not getting out of the penalty box")
    ...
    fun wasCorrectlyAnswered(): Boolean {
        if (inPenaltyBox[currentPlayer]) {
            ...
            presenter.present("Answer was corrent!!!!")
            purses[currentPlayer]++
            presenter.present(
                players[currentPlayer].toString()
                        + " now has "
    ...
    fun wrongAnswer(): Boolean {
        ...
        presenter.present(players[currentPlayer].toString() + " was sent to the penalty box")
